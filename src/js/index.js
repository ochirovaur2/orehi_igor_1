...
function sellPrice(){
  ...
  let arr = document.getElementsByClassName("btn__shopping-card-js");
  arr = Array.from(arr);
  let parentNode = "";
  for(let i = 0; i < arr.length; i++){
    parentNode = arr[i].parentNode.parentNode;
    let priceBox = $(parentNode).find('.cards__price-box');
    let price = $(parentNode).find('.btn__shopping-card').attr('data-price');
    price = +price.replace(/,/, '.');
    let sell = arr[i].dataset.sell;
    if(sell != 0){
      sell = +sell / 100;
    ...
    if(sell != 1) {
      ...
      let markup = ` 
          <p><span class="paragraph_price cards__old-price">${moneyFormat(price)}</span> руб.</p>
          <p><span class="paragraph_price cards__new-price">${moneyFormat(sellPrice)}</span> руб.</p>
...
jQuery(document).ready(function($){
  let radioBtn = $('.cards__radio');
  

  radioBtn.on('click', function(event){
    let btn = event.target;
    let parentNode = event.currentTarget.parentNode.parentNode.parentNode.parentNode;
    let radioVal = $(btn).val() ;
    let trigger = $(btn).attr('data-trigger');
    let price = "";
    
    let priceBox = $(parentNode).find('.cards__price-box');
    let sell = $(parentNode).find('.btn__shopping-card').attr('data-sell');
    ...
    if(sell != 1) {
      ...
      let markup = ` 
          <p><span class="paragraph_price ">${moneyFormat(price)}</span> руб.</p>
        `
      $(priceBox).html(markup); 
    }
